...
export async function up(knex) {
  await knex.schema.createTable(TABLE_NAME, function(table) {
    ...
    table.uuid('moduleId').nullable().references('modules.id');
    table.string('shortId', 8).notNullable().unique().comment('used for permanent url');
    ...
    table.check('?? IS NULL OR ?? = ??', [
      'moduleId',
      'id',
      'moduleId',
    ]);
    table.unique('moduleId', { predicate: knex.whereNotNull('moduleId') });
  });
}
