...
function _validateMissionIntroductionMedia(mission) {
  if (!_.isNull(mission.introductionMediaUrl) && _.isNull(mission.introductionMediaType)) {
    throw new MissionIntroductionMediaError(
...
async function _validateMissionContent(mission) {
  ...
  if (mission.status === Mission.status.VALIDATED) {
    if (!mission.thematicIds)
      throw new InvalidMissionContentError("La mission ne peut pas être mise à jour car elle n'a pas de thématique");
    ...
    if (tubeIds.length === 0)
      throw new InvalidMissionContentError("La mission ne peut pas être mise à jour car elle n'a pas de sujet");
    ...
    for (const tubeId of tubeIds) {
      ...
      for (let level = 1; level <= maxlevel; level++) {
        ...
        if (activeSkillByLevel.length === 0) {
          ...
          if (skillsByLevel.filter((skill) => skill.isEnConstruction).length > 0)
            warnings.push(`L\'activité \'${tube.name}\' n\'a pas d\'acquis actif pour le niveau ${level}.`);
        }
      }
    }
  }
