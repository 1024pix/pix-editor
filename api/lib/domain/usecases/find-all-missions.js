...
export async function findAllMissions(
  params,
  dependencies = {
    missionRepository,
  ...
  const competences = await dependencies.competenceRepository.getMany(
    result.missions.map((mission) => mission.competenceId),
  );
  ...
  const missionSummmaries = result.missions.map((mission) => {
    ...
    return new MissionSummary({
      ...
      competence: missionCompetence
        ? `${missionCompetence.index} ${missionCompetence.name_i18n.fr}`
        : `Compétence non trouvée : ${mission.competenceId}`,
    });
  });
