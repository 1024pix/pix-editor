...
const elementSchema = Joi.alternatives().conditional('.type', { switch: ALLOWED_ELEMENTS_SCHEMA });

const stepperElementSchema = Joi.alternatives().conditional('.type', { switch: ALLOWED_ELEMENTS_SCHEMA.filter((elementSchema) => !ELEMENTS_FORBIDDEN_IN_STEPPER.includes(elementSchema.is)) });
...
const componentStepperSchema = Joi.object({
  ...
  steps: Joi.array()
    .items(
      Joi.object({ elements: Joi.array().items(stepperElementSchema).required() }),
    )
    .min(2)
    .required(),
});
...
const grainSchema = Joi.object({
  ...
  components: Joi.array()
    .items(
      Joi.alternatives().conditional('.type', { switch: [{ is: 'element', then: componentElementSchema }, { is: 'stepper', then: componentStepperSchema }] }),
    )
...
const moduleGlossaryEntrySchema = Joi.object({
