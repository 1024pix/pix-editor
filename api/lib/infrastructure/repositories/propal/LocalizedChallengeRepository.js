...
export class LocalizedChallengeRepository extends KnexRepository {
  async list() {
    const localizedChallengeDtos = await _queryLocalizedChallengeWithAttachment(this.dbConn)
      .orderBy('id');
    return localizedChallengeDtos.map(_toDomain);
  }

  async create({ localizedChallenges = [], generateId = _generateId }) {
    if (localizedChallenges.length === 0) {
      return;
    }
    const dataToInsert = _adaptModelsForDB(localizedChallenges, generateId);
    await this.dbConn('localized_challenges').insert(dataToInsert).onConflict().ignore();
  }

  async getByChallengeIdAndLocale({ challengeId, locale }) {
    const dto = await _queryLocalizedChallengeWithAttachment(this.dbConn)
      .where({
  ...
  async listByChallengeIds({ challengeIds }) {
    const dtos = await _queryLocalizedChallengeWithAttachment(this.dbConn)
  ...
  async update({
    ...
    const [dto] = await this.dbConn('localized_challenges').where('id', id).update({
      locale,
      embedUrl,
      status,
      geography,
    ...
    const [primaryEmbedUrl] = await this.dbConn('localized_challenges').where({ id: dto.challengeId }).pluck('embedUrl');
...
function _queryLocalizedChallengeWithAttachment(knexConnection) {
