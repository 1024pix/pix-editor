...
describe('Unit | Infrastructure | Datasource | Airtable | AttachmentDatasource', () => {
  describe('#filterByLocalizedChallengeId', () => {
    it('calls airtable', async () => {
      ...
      const airtableFindRecordsSpy = vi.spyOn(airtable, 'findRecords').mockResolvedValue([attachmentRecord]);
      ...
      expect(airtableFindRecordsSpy).toHaveBeenCalledWith('Attachments', {
        filterByFormula: '{localizedChallengeId} = "recChallenge"',
      });
    });
  });
});
