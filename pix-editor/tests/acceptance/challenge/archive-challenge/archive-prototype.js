...
module('Acceptance | Archive prototype', function (hooks) {
  ...
  module('When skill has no other prototype', function () {
    test('it should archive skill and alternative', async function (assert) {
      // when
      ...
      await click(await screen.getByRole('button', { name: 'Archiver' }));
      assert.ok(await screen.findByRole('heading', { name: 'Archivage' }));
      ...
      assert.deepEqual(pixToastSendSuccess.args[0], ['Déclinaison n°1 archivée']);
      assert.deepEqual(pixToastSendSuccess.args[1], ['Épreuve archivée']);
      assert.strictEqual(updatedPrototype.status, 'archivé');
      assert.strictEqual(updatedAlternative.status, 'archivé');
  ...
  module('When skill has an other proposed prototype', function (hooks) {
    ...
    test('it should archive alternative and deactivate skill', async function (assert) {
      // when
      const screen = await visit(`competence/${competence.id}/prototypes/${prototype.id}?view=production`);
