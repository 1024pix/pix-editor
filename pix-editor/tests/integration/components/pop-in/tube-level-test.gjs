...
module('Integration | Component | popin-tube-level', function (hooks) {
  ...
  hooks.beforeEach(async function () {
    ...
    await render(
      <template>
        <TubeLevel
          @setTubeLevel={{self.setTubeAction}}
          @clearTube={{self.clearTubeAction}}
          @skills={{self.tubeSkills}}
          @level={{self.selectedTubeLevel}}
          @selectedSkills={{self.selectedTubeSkills}}
          @tube={{self.selectedTube}}
          @isThematicResultMode={{self.isThematicResultMode}}
          @close={{self.closeTubeLevel}}
        />
      </template>,
    );
  });
