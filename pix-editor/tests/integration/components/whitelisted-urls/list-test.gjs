...
import WhitelistedUrlsList from '../../../../components/whitelisted-urls/list';
...
module('Integration | Component | whitelisted-urls/list', function(hooks) {
  ...
  const urlFilterValue = 'initialUrlValue';
  const namesFilterValue = 'initialNamesValue';
  ...
  test('it should display list of whitelisted urls passed in params and initialize filter inputs', async function(assert) {
    ...
    const onApplyFiltersClicked = onApplyFiltersClickedStub;
    const onClearFiltersClicked = onClearFiltersClickedStub;

    // when
    const screen = await render(<template>
      <WhitelistedUrlsList
        @whitelistedUrls={{whitelistedUrls}}
        @urlFilterValue={{urlFilterValue}}
        @namesFilterValue={{namesFilterValue}}
        @namesFilterValue={{namesFilterValue}}
  ...
  test('it should pass delete item arg function to WhitelistedUrlsTable', async function(assert) {
    ...
    const onApplyFiltersClicked = onApplyFiltersClickedStub;
    const onClearFiltersClicked = onClearFiltersClickedStub;
    const onDeleteItemClicked = sinon.stub();
    ...
    // then
    assert.ok(onDeleteItemClicked.calledOnce);
  });
});
