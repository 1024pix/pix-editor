...
import WhitelistedUrlsTable from '../../../../components/whitelisted-urls/table';
import { setupIntlRenderingTest } from '../../../setup-intl-rendering';

module('Integration | Component | whitelisted-urls/table', function(hooks) {
  ...
  test('it should display list of whitelisted urls passed in params', async function(assert) {
    // given
    const whitelistedUrls = [whitelistedUrl1, whitelistedUrl2];

    // when
    const screen = await render(<template>
      <WhitelistedUrlsTable @whitelistedUrls={{whitelistedUrls}}/>
    </template>);
  ...
  test('it should copy url to clipboard when clicking on copy button', async function(assert) {
    // given
    const whitelistedUrls = [whitelistedUrl1];
    const clipboard = {
    ...
    // then
    await screen.getByRole('button', { name: 'Copier l\'URL whitelistée' }).click();
    assert.ok(clipboard.writeText.calledWith('https://foo.com'));
  });

  test('it should call arg function when clicking on delete button', async function(assert) {
    // given
    const whitelistedUrls = [whitelistedUrl1];
    const onDeleteWhitelistedUrl = sinon.stub();

    // when
    const screen = await render(<template>
      <WhitelistedUrlsTable
        @whitelistedUrls={{whitelistedUrls}}
        @onDeleteWhitelistedUrl={{onDeleteWhitelistedUrl}}
      /></template>);

    // then
    await screen.getByRole('button', { name: 'Supprimer l\'URL de la whitelist' }).click();
    assert.ok(onDeleteWhitelistedUrl.calledOnce);
  });
});
