...
<template><form action class="ui form challenge-form">
  <Mde @title="Consigne" @value={{@challenge.instruction}} @edition={{@edition}} @helpContent={{this.helpInstructions}} @setValue={{fn (mut @challenge.instruction)}} />
  <ToggleField @edition={{@edition}} @model={{@challenge}} @modelField="alternativeInstruction" @hideTextButton="Supprimer l'alternative textuelle" @displayTextButton="Ajouter une alternative textuelle" @confirmText="l'alternative textuelle" @displayField={{@displayAlternativeInstructionsField}} @setDisplayField={{@setDisplayAlternativeInstructionsField}}>
    <Mde @title="Alternative textuelle" @value={{@challenge.alternativeInstruction}} @edition={{@edition}} @helpContent={{this.helpInstructions}} @setValue={{fn (mut @challenge.alternativeInstruction)}} data-test-alternative-instructions-field />
  </ToggleField>
  {{#if @challenge.isPrototype}}
    <PixSelect @options={{this.options.types}} @onChange={{this.setChallengeType}} @value={{this.challengeTypeValue}} @isDisabled={{not @edition}} @hideDefaultOption={{true}}>
      <:label>Modalité</:label>
    </PixSelect>
    <div class="field">
      <Checkbox @label="Sans validation (Pix Junior)" @checked={{@challenge.noValidationNeeded}} @disabled={{not @edition}} data-test-no-validation-needed-checkbox={{@challenge.id}} />
    </div>
    {{#if this.typeIsQCUOrQCM}}
      <Checkbox @label="Afficher aléatoirement l'ordre des propositions" @checked={{@challenge.shuffled}} @disabled={{not @edition}} data-test-checkbox-shuffle />
    {{/if}}
  {{/if}}
  {{#if (and this.typeIsQROCOrQROCMInd (not this.isAutoReply))}}
    <PixSelect @options={{this.options.format}} @onChange={{fn (mut @challenge.format)}} @value={{this.challengeFormatValue}} @isDisabled={{not @edition}} @hideDefaultOption={{true}}>
      <:label>Format QROC</:label>
    </PixSelect>
  {{/if}}
  {{#unless this.isAutoReply}}
    <Mde @title="Propositions" @value={{@challenge.proposals}} @setValue={{fn (mut @challenge.proposals)}} @edition={{@edition}} @helpContent={{this.helpSuggestions}} data-test-suggestion-field />
  {{/unless}}
  <Textarea @title="Réponses" @value={{@challenge.solution}} @edition={{@edition}} @helpContent={{this.helpAnswers}} data-test-answers-field @id="challenge-solution" />
  {{#if (and @challenge.isTextBased (not this.isAutoReply))}}
    <div id="toleranceField" data-test-tolerence-fields class="field {{if @edition "" "disabled"}}">
      ...
      <div class="three fields">
        <div class="field">
          <Checkbox @label="T1 (espaces/casse/accents)" @checked={{@challenge.t1Status}} @disabled={{not @edition}} />
        </div>
        <div class="field">
  ...
  {{/if}}
  <ToggleField @edition={{@edition}} @model={{@challenge}} @modelField="solutionToDisplay" @hideTextButton="Supprimer la bonne réponse à afficher" @displayTextButton="Ajouter une bonne réponse à afficher" @confirmText="la bonne réponse à afficher" @displayField={{@displaySolutionToDisplayField}} @setDisplayField={{@setDisplaySolutionToDisplayField}}>
    <Textarea @title="Bonne réponse à afficher" @value={{@challenge.solutionToDisplay}} @edition={{@edition}} data-test-solution-to-display-field @id="challenge-solution-to-display" />
  </ToggleField>

  <ToggleField @edition={{@edition}} @model={{@challenge}} @modelField="urlsToConsult" @hideTextButton="Supprimer des URLs nécessaires à la résolution de l'épreuve" @displayTextButton="Ajouter des URLs nécessaires à la résolution de l'épreuve" @confirmText="URLs externes nécessaires à la résolution de l'épreuve" @displayField={{@displayUrlsToConsultField}} @setDisplayField={{@setDisplayUrlsToConsultField}} @textToolTip="Ces URLs doivent être trouvées par l’utilisateur car elles ne sont communiquées ni dans la consigne ni dans les propositions.">
    <Textarea @title="URLs externes nécessaires à la résolution de l'épreuve" @value={{@urlsToConsult}} @edition={{@edition}} @change={{@setUrlsToConsult}} @helpContent={{this.helpUrlsToConsult}} data-test-urls-to-consult-field @id="urls-to-consult-to-display" />
  </ToggleField>
  ...
  <Illustration @title="Illustration" @value={{@challenge.illustration}} @edition={{@edition}} @addIllustration={{this.addIllustration}} @removeIllustration={{@removeIllustration}} @display={{@showIllustration}} data-test-file-input-illustration />
  {{#if @challenge.illustration}}
    <Textarea @value={{@challenge.illustrationAlt}} @title="Texte alternatif" @edition={{@edition}} @id="challenge-illustration-alt" />
  {{/if}}
  <Files @title="Pièces jointes" @value={{@challenge.piecesJointes}} @baseName={{@challenge.attachmentBaseName}} @addAttachment={{this.addAttachment}} @edition={{@edition}} @removeAttachment={{@removeAttachment}} data-test-file-input-attachment />
  <Input @title="Embed" @value={{@challenge.embedURL}} @edition={{@edition}} @label="URL" @id="challenge-embed-url" @change={{@checkEmbedURL}} />
  {{#if @invalidEmbedURL}}
  ...
  {{#if @challenge.isPrototype}}
    <div class="fields--selectors">
      <div class="field">
        <Checkbox @label="Validation par l'embed (Pix Junior)" @checked={{@challenge.hasEmbedInternalValidation}} @disabled={{not @edition}} data-test-has-embed-internal-validation-checkbox={{@challenge.id}} />
      </div>
    </div>
    <PixSelect @options={{this.options.pedagogy}} @onChange={{fn (mut @challenge.pedagogy)}} @value={{@challenge.pedagogy}} @isDisabled={{not @edition}} @hideDefaultOption={{true}}>
      <:label>Type pédagogie</:label>
    </PixSelect>
    <PixSelect @options={{this.options.declinable}} @onChange={{fn (mut @challenge.declinable)}} @value={{@challenge.declinable}} @isDisabled={{not @edition}} @hideDefaultOption={{true}}>
  ...
  <div class="field {{if @edition "" "disabled"}}">
    ...
    <div class="two fields">
      <div class="field">
        <PixMultiSelect @placeholder="Choisir une ou plusieurs langues" @onChange={{this.setLocales}} @emptyMessage="Aucune langue sélectionnée" @values={{this.languages}} @options={{this.languageOptions}} @isDisabled={{not @edition}}>
          <:label>Langue(s)</:label>
          <:default as |option|>{{option.label}}</:default>
        </PixMultiSelect>
      </div>
      <div class="field">
        <PixSelect @id="challenge-select-geography" @placeholder="Géographie" @onChange={{fn (mut @challenge.geography)}} @value={{this.challengeGeographyValue}} @options={{@countries}} @isDisabled={{not @edition}} @hideDefaultOption={{true}}>
    ...
    {{#if @challenge.isPrototype}}
      <div class="field">
        <PixMultiSelect @placeholder="Champs contextualisés" @onChange={{this.setContextualizedFields}} @emptyMessage="Aucun champ sélectionné" @values={{this.args.challenge.contextualizedFields}} @options={{this.options.contextualizedFields}} @isDisabled={{not @edition}}>
