...
const helpUrlsToConsult = '<p>Séparer les liens par un retour à la ligne</p>';
...
export default class ChallengeForm extends Component {
  ...
  get geographyOptionList() {
    return options.geography.map((g) => ({ label: g, value: g }));
  }
  ...
  get challengeTypeValue() {
    ...
    return options.types.find((type) => type.value === actualType).value;
  }
  ...
  get challengeFormatValue() {
    ...
    return options.format.find((format) => format.value === this.args.challenge.format).value;
  }
