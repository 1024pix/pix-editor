...
<template><EmberTable as |t|>
  <t.head @columns={{this.headers}} @widthConstraint={{"gte-container"}} @resizeMode={{"fluid"}} @onUpdateSorts={{this.sortBy}} @sorts={{this.sorts}} @sortFunction={{this.sort}} />
  <t.body @renderAll={{this.renderAll}} @rows={{@list}} @rowSelectionMode={{"single"}} @checkboxSelectionMode={{"none"}} @onSelect={{this.selectRow}} @selection={{this.current}} as |b|>
    <b.row as |r|>
