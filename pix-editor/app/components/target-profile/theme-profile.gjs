...
<template><div data-test-theme-profile class="row theme-profile">
  ...
  <div class="thirteen wide column">
    {{#each this.filteredTubes as |tube|}}
      {{#if @isThematicResultMode}}
        <TubeProfile class="tube-profile thematicResult" @tube={{tube}} @clickAction={{this.clickOnThematicResultTube}} @selectedSkillLevel={{tube.selectedThematicResultLevel}} @showTubeDetails={{@showTubeDetails}} />
      {{else}}
        <TubeProfile class="tube-profile" @tube={{tube}} @clickAction={{this.clickOnTube}} @selectedSkillLevel={{tube.selectedLevel}} @showTubeDetails={{@showTubeDetails}} />
      {{/if}}
    {{/each}}
  </div>
</div></template>
