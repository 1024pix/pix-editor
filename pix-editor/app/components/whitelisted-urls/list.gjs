...
<template><PixFilterBanner @title="Filtres" class="table-filter-banner filter-whitelisted-url-form" @clearFiltersLabel="Réinitialiser les filtres" @onClearFilters={{this.clearFilters}} @isClearFilterButtonDisabled={{false}} @loadFiltersLabel="Filtrer" @onLoadFilters={{this.applyFilters}}>
  <PixInput @id="whitelisted-url-filter-names" placeholder="Nom d'acquis" @screenReaderOnly={{true}} @value={{@namesFilterValue}} {{on "change" this.updateNamesFilterValue}}>
    <:label>Nom d'acquis</:label>
  </PixInput>
  <PixInput @id="whitelisted-url-filter-url" placeholder="URL" @screenReaderOnly={{true}} @value={{@urlFilterValue}} {{on "change" this.updateUrlFilterValue}}>
    <:label>URL</:label>
  </PixInput>
</PixFilterBanner>
<div class="whitelisted-urls-table">
  <PixTable @caption="Liste des URLs à ne pas mettre dans les URLs cassées" @condensed={{true}} @data={{@whitelistedUrls}} @variant="primary" @onRowClick={{@goToEditWhitelistedUrl}}>
        ...
        <:cell>
          <div class="actions">
            <PixTooltip @id="copy-whitelisted-url-link-tooltip" @position="left" @isInline={{true}}>
              <:triggerElement>
                <PixIconButton @triggerAction={{fn this.copyUrl whitelistedUrl}} @iconName="copy" @iconPrefix="far" @ariaLabel="Copier l'URL" class="icon" aria-describedby="copy-whitelisted-url-link-tooltip" />
              </:triggerElement>
              <:tooltip>Copier l’URL</:tooltip>
            </PixTooltip>
            <PixTooltip @id="delete-whitelisted-url-tooltip" @position="left" @isInline={{true}}>
              <:triggerElement>
                <PixIconButton @triggerAction={{fn this.deleteWhitelistedUrl whitelistedUrl}} @iconName="delete" @ariaLabel="Supprimer l'URL" class="icon" aria-describedby="delete-whitelisted-url-tooltip" />
              </:triggerElement>
