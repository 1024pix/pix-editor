...
import VuePixTable from './pix-table.dist.js';
...
export default class PixTable extends Component {
  ...
  mountVue(element) {
    this.vueApp = createApp({
      render: () => h(VuePixTable, this.propsData),
    });
  ...
  <template>
    <div
      ...
      {{didUpdate (fn this.updateProps "variant") @variant}}
      {{didUpdate (fn this.updateProps "caption") @caption}}
      {{didUpdate (fn this.updateProps "condensed") @condensed}}
      {{didUpdate (fn this.updateProps "displayCaption") @displayCaption}}
      {{didUpdate (fn this.updateProps "data") @data}}
    ></div>
  </template>
}
