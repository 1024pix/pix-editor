...
<template>{{#if (eq this.cellType "production")}}
...
{{else if (eq this.cellType "add-skill")}}
  <td>
    <LinkTo @route="authenticated.competence.skills.new" @models={{array @tube.id this.skillLevel}} class="add-skill" aria-label="{{concat "ajouter un acquis de niveau " this.skillLevel " pour le sujet " @tube.name}}">
