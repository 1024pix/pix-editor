...
<template><div class="competence-grid">
  <table class="ui celled table definition tubes">
    <tbody>
    {{#each this.thematicOverviewsOrThematics as |thematicOverviewOrThematic|}}
      <CompetenceGridThematic @languageFilter={{@languageFilter}} @section={{@section}} @view={{@view}} @thematic={{thematicOverviewOrThematic.thematic}} @thematicOverview={{thematicOverviewOrThematic.thematicOverview}} @newTube={{@newTube}} @displaySortTubesPopIn={{@displaySortTubesPopIn}} @link={{@link}} />
    {{/each}}
    </tbody>
  </table>
</div>
