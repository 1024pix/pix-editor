...
export default class CompetenceCompetenceGridTubeComponent extends Component {
  <template>
    <td data-test-tube-cell class="tube-cell">
      {{#if (eq @section "skills")}}
        <LinkTo
          data-test-tube-managment
          @route="authenticated.competence.tubes.single"
          @model={{@tube}}
        >{{this.name}}</LinkTo>
      {{else}}
        {{this.name}}
    ...
    {{#each this.skillOverviewsOrSkills as |skillOverviewOrSkill index|}}
      <GridCell
        ...
        @view={{@view}}
        @skill={{skillOverviewOrSkill.skill}}
        @skills={{skillOverviewOrSkill.skills}}
        @skillOverview={{skillOverviewOrSkill.skillOverview}}
        @link={{@link}}
        @tube={{@tube}}
        @tubeId={{@tubeOverview.airtableId}}
        @index={{index}}
      />
    {{/each}}
  </template>
  ...
  get name() {
    return this.isOverview ? this.args.tubeOverview.name : this.args.tube.name;
  }
}
