...
<template><div class="main-left">
  <div class="main-title">
    <h1 class="ui header">{{if @controller.isThematicResultMode "Générateur de résultat thématique" "Générateur de profil cible"}}
      <div class="target-profile-filter {{if @controller.isThematicResultMode "active"}}">
        Résultat thématique
        <div class="ui toggle checkbox ">
          <Input id="thematicResult" @type="checkbox" @checked={{@controller.isThematicResultMode}} {{on "change" @controller.toggleThematicResult}} class="toggle" />
    ...
    <div class="row">
      ...
      {{else}}
        <div class="three wide column">
          <PixMultiSelect @options={{@controller.frameworkOptionList}} @values={{@controller.selectedFrameworkIds}} @onChange={{@controller.selectFrameworks}} @placeholder={{"Aucun référentiel sélectionné"}}>
            <:label>
  ...
  <div class="ui attached target-profile">
    {{#each @controller.areas as |area|}}
      <AreaProfile @area={{area}} @displayTube={{@controller.displayTube}} @displayThematicResultTube={{@controller.displayThematicResultTube}} @level={{@controller.selectedTubeLevel}} @selectedSkills={{@controller.selectedTubeSkills}} @showTubeDetails={{@controller.showTubeDetails}} @clearTube={{@controller.unsetProfileTube}} @setTubeLevel={{@controller.setProfileTube}} @filter={{@controller.filter}} @isThematicResultMode={{@controller.isThematicResultMode}} />
    {{/each}}
    <Input @type="file" id="target-profile__open-file" {{on "change" @controller.openFile}} />
  </div>
</div>
<TubeLevel @setTubeLevel={{@controller.setTubeAction}} @clearTube={{@controller.clearTubeAction}} @skills={{@controller.tubeSkills}} @level={{@controller.selectedTubeLevel}} @selectedSkills={{@controller.selectedTubeSkills}} @tube={{@controller.selectedTube}} @isThematicResultMode={{@controller.isThematicResultMode}} @close={{@controller.closeTubeLevel}} @showModal={{@controller.displayTubeLevel}} />
<SingleEntry @title={{@controller.singleEntryPopInTitle}} @label={{@controller.singleEntryPopInLabel}} @setValue={{@controller.singleEntryPopInAction}} @close={{@controller.closeSingleEntry}} @showModal={{@controller.displaySingleEntry}} />
<ThresholdCalculation @title="Paliers indicatifs" @close={{@controller.closeThresholdCalculation}} @model={{@controller.areas}} @showModal={{@controller.displayThresholdCalculation}} />
</template>
