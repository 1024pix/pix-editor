...
<template><header class="page-header">
  ...
  <div class="page-actions">
    <PixButtonLink @backgroundColor="blue" @route="authenticated.static-courses.static-course.edit" @isDisabled={{not @controller.canEditStaticCourse}} class="pix-button-link-with-icon white-font">
      <PixIcon @name="edit" @ariaHidden={{true}} />
      Éditer le test statique
    </PixButtonLink>
    <PixButton @backgroundColor="grey" @isBorderVisible={{true}} @triggerAction={{@controller.showActivationModal}} @isDisabled={{not @controller.canDeactivateOrReactivateStaticCourse}}>
...
<main class="page-body">
  <section class="page-section">
      ...
      <ul>
        ...
        <li><span class="bold">Statut du test : </span>
          ...
          {{#if @controller.model.staticCourse.deactivationReason}}
            (Motif: {{@controller.model.staticCourse.deactivationReason}})
          {{/if}}
        </li>
        <li><span class="bold">Crée le : </span>{{formatDate @controller.model.staticCourse.createdAt "DD/MM/YYYY" allow-empty=true}}</li>
        <li><span class="bold">Dernière modification : </span>{{formatDate @controller.model.staticCourse.updatedAt "DD/MM/YYYY" allow-empty=true}}</li>
      </ul>
      <div class="static-course-details__card-information--actions">
        <PixButton @triggerAction={{fn @controller.copyStaticCoursePreviewUrl @controller.model.staticCourse}} @iconBefore="copy" @plainIconBefore="far" @backgroundColor="transparent-light" @isBorderVisible={{true}} @isDisabled={{not @controller.model.staticCourse.isActive}}>
          Copier le lien
        </PixButton>
        <PixButtonLink @href={{if @controller.model.staticCourse.isActive @controller.model.staticCourse.previewURL "#"}} target={{if @controller.model.staticCourse.isActive "_blank" "_self"}} rel="noopener noreferrer" @backgroundColor="transparent-light" @isBorderVisible={{true}} class="pix-button-link-with-icon" @isDisabled={{not @controller.model.staticCourse.isActive}}>
    ...
    <div class="page-actions">
      <PixButtonLink @backgroundColor="transparent-light" @route="authenticated.static-courses.list" @isBorderVisible={{true}}>
        Retour
      </PixButtonLink>
      <PixButtonLink @backgroundColor="blue" @route="authenticated.static-courses.static-course.edit" @isDisabled={{not @controller.canEditStaticCourse}} class="pix-button-link-with-icon white-font">
        <PixIcon @name="edit" />
...
<PixModal @title="Désactivation" @onCloseButtonClick={{@controller.closeDeactivationModal}} @showModal={{@controller.shouldDisplayDeactivationModal}}>
  <:content>
    <p>
      Êtes-vous sûr de vouloir désactiver le test statique <b>{{@controller.model.staticCourse.name}}</b> ?<br />
      <PixInput @id="deactivationReason" @value={{@controller.deactivationReason}} {{on "input" @controller.setDeactivationReason}}><:label>Raison de désactivation (facultatif)</:label></PixInput>
    </p>
  </:content>
  <:footer>
    <PixButton @backgroundColor="transparent-light" @isBorderVisible={{true}} @triggerAction={{@controller.closeDeactivationModal}}>
      Non
    </PixButton>
...
<PixModal @title="Réactivation" @onCloseButtonClick={{@controller.closeReactivationModal}} @showModal={{@controller.shouldDisplayReactivationModal}}>
  <:content>
    <p>
      Êtes-vous sûr de vouloir réactiver le test statique <b>{{@controller.model.staticCourse.name}}</b> ?
    </p>
  </:content>
  <:footer>
    <PixButton @backgroundColor="transparent-light" @isBorderVisible={{true}} @triggerAction={{@controller.closeReactivationModal}}>
      Non
    </PixButton>
