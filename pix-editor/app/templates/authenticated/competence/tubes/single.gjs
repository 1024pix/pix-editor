  ...
  <div class="ui menu">
    <div class="ui left menu">
      {{#if (and @controller.mayMove (not @controller.creation) (not @controller.edition))}}
        <button class="ui button icon item" {{on "click" @controller.selectCompetence}} type="button"><i class="icon random"></i></button>
      {{/if}}
    </div>
    ...
    <div class="ui right menu">
      {{#if @controller.maximized}}
        <button class="ui button icon item" {{on "click" @controller.minimize}} type="button"><i class="window minimize icon"></i></button>
      {{else}}
        <button class="ui button icon item" {{on "click" @controller.maximize}} type="button"><i class="window maximize outline icon"></i></button>
      {{/if}}
...
<div class="tube-details">
  ...
  <div class="ui vertical compact labeled icon menu tube-menu">
    {{#if @controller.edition}}
      <button class="ui button item important-action {{if @controller.disableSaveButton " disabled"}}" {{on "click" @controller.save}} type="button">
        <i class="save icon"></i>
        Enregistrer
      </button>
...
<SelectLocation @variant="tube" @class="tube-select-competence" @onSubmit={{@controller.setCompetence}} @title={{concat "Déplacer le tube " @controller.tube.name " dans une autre thématique"}} @theme={{@controller.tube.theme}} @close={{@controller.closeSelectCompetence}} @showModal={{@controller.displaySelectLocation}} />
</template>
