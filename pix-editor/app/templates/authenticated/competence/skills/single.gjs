...
<template><div class="skill-header {{@controller.skill.statusCSS}}">
  <div class="ui menu">
    <div class="ui left menu">
      {{#unless @controller.edition}}
        {{#if @controller.mayDuplicate}}
          <button class="ui icon button" {{on "click" @controller.duplicateSkill}} type="button" title="Dupliquer vers"><i class="icon copy"></i></button>
        {{/if}}
        <LinkTo class="ui button icon item" title="Liste des versions" @route="authenticated.competence.skills.list" @models={{array @controller.skill.tube.id @controller.skill.level}}>
        ...
        {{#if (or @controller.mayArchive @controller.mayObsolete)}}
          <div class="skill-status-actions">
            <PixIconButton @ariaLabel="Changer le statut de l'acquis" @iconName="bolt" @plainIcon={{true}} @triggerAction={{@controller.toggleStatusActionMenu}} {{on "focusout" @controller.hideStatusActionMenu}} />
    ...
    <div class="ui right menu">
      {{#if @controller.maximized}}
        <button class="ui icon button" {{on "click" @controller.minimize}} type="button"><i class="window minimize icon"></i>
        </button>
      {{else}}
        <button class="ui icon button" {{on "click" @controller.maximize}} type="button"><i class="window maximize outline icon"></i></button>
      {{/if}}
      <button class="ui icon button" {{on "click" @controller.close}} type="button"><i class="icon window close"></i></button>
    </div>
  </div>
</div>
  ...
  <div class="ui vertical compact labeled icon menu skill-menu">
    {{#if @controller.edition}}
      <button class="ui button important-action item" {{on "click" @controller.save}} type="button">
        <i class="save icon"></i>
        Enregistrer <span class="sr-only">l'acquis {{@controller.skill.name}}</span>
      </button>
...
<SelectLocation @variant="skill" @onSubmit={{@controller.duplicateToLocation}} @title={{concat "Destination de la copie de " @controller.skill.name}} @tube={{@controller.skill.tube}} @close={{@controller.closeSelectLocation}} @showModal={{@controller.displaySelectLocation}} />
<Changelog @onApprove={{@controller.approveChangelog}} @defaultValue={{@controller.changelogText}} @showModal={{@controller.displayChangeLog}} />
<ConfirmLog @title={{t "common.confirm-log.save"}} @onApprove={{@controller.saveSkillCallBack}} @defaultValue={{@controller.defaultSaveChangelog}} @inputId="changelog-message" @onDeny={{@controller.closeComfirmLogPopin}} @content={{t "common.confirm-log.content"}} @label={{t "common.confirm-log.label"}} @showModal={{@controller.displayConfirmLog}} />
{{outlet}}
</template>
