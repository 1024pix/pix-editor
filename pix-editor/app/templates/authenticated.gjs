...
<template><div class="ui container fluid application">
      ...
      {{#if @controller.messages.length}}
        <div class="messages">
          {{#each @controller.messages as |message|}}
            <div data-test-main-message class={{concat "ui floating message " (if message.positive "positive" "warning")}} id={{message.id}}>
              <p>
                {{#if message.positive}}
  ...
  <Logout @onDeny={{@controller.closeLogout}} @onConfirm={{@controller.logout}} @showModal={{@controller.displayLogout}} @class="popin-logout" />
  <Confirm @title={{@controller.confirmTitle}} @content={{@controller.confirmContent}} @onApprove={{@controller.confirmApprove}} @onDeny={{@controller.confirmDeny}} @showModal={{@controller.displayConfirm}} />
</div>
</template>
